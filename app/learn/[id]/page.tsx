...
export default function LessonPage({ params }: { params: { id: string } }) {
  ...
  // RESULTS SCREEN - proper return, not inline JSX
  if (showResults && quizFinished) {
    return (
      <div className="p-4 pt-8 md:p-8 max-w-2xl mx-auto space-y-6 pb-32">
        <button onClick={() => { window.location.href = '/learn'; }} className="flex items-center gap-2 text-sm text-white/50 hover:text-white transition-colors group">
          <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
          Back to Library
        </button>
        ...
        <div className="text-center space-y-4">
          <motion.div
            ...
            animate={{ scale: 1 }}
            transition={{ type: 'spring', stiffness: 300, damping: 20 }}
            className="w-24 h-24 bg-[#00C896]/20 text-[#00C896] rounded-full flex items-center justify-center mx-auto"
          >
        ...
        <div className="space-y-4">
          {lesson.quiz.map((q, i) => {
            const isCorrect = q.correctIndex === quizScores[i];
            return (
              <div key={i} className={"bg-surface border p-4 rounded-2xl " + (isCorrect ? 'border-[#00C896]/30' : 'border-[#FF6B6B]/30')}>
                ...
                <div className="space-y-2">
                  {q.options.map((opt, idx) => {
                    ...
                    const isCorrectlySelected = isCorrectOpt && isSelected;
                    const isIncorrectlySelected = !isCorrectOpt && isSelected;
                      ...
                      <div
                        ...
                        className={`w-full text-left p-3 rounded-xl border text-sm transition-colors ` +
                          (isCorrectlySelected ? 'bg-[#00C896]/20 border-[#00C896] text-[#00C896] font-bold' :
                           isIncorrectlySelected ? 'bg-[#FF6B6B]/20 border-[#FF6B6B] text-[#FF6B6B]' :
                           isCorrectOpt ? 'bg-[#00C896]/10 border-[#00C896]/30 text-[#00C896]' :
        ...
        <div className="flex items-center justify-center gap-2 bg-[#00C896]/10 border border-[#00C896]/30 rounded-xl px-4 py-3">
          <Sparkles className="w-5 h-5 text-[#00C896]" />
          <span className="text-white font-bold">+{earnedXp} XP {earnedXp > 0 ? 'awarded' : '(get all correct to earn XP)'}</span>
        </div>
        ...
        <div className="flex gap-3 mt-4">
          <button onClick={() => { window.location.href = '/learn'; }} className="bg-white text-black font-bold py-3 px-8 rounded-xl">Back to Library</button>
